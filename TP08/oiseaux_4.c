...
void change_amities(struct nuee *nuee)
{
    for (int i = 0; i < nuee->taille; i++)
    {
        if (rand() % 10 == 0)
        {
            ...
            // Oiseau le plus proche
            double dist = HUGE_VAL;
            int ennemi = 0;
            for (int j = 0; j < nuee->taille; j++)
            {
                if (j != i)
                {
                    double x = nuee->cui[i].x - nuee->cui[j].x;
                    double y = nuee->cui[i].y - nuee->cui[j].y;
                    double d = x * x + y * y; // On compare le carré des distances pour éviter les racines carrées (qui sont lentes)
                    if (d < dist)
                    {
                        dist = d;
                        ennemi = j;
